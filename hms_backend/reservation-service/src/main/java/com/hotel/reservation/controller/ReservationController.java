...
public class ReservationController {
    ...
    public ResponseEntity<ApiResponse<ReservationResponse>> updatePaymentStatus(
        ...
        try {
            ReservationResponse reservation = reservationService.updatePaymentStatus(reservationId, status);
            return ResponseEntity.ok(
                    new ApiResponse<>(true, "Cập nhật trạng thái thanh toán thành công", reservation)
            );
        } catch (IllegalArgumentException e) {
            return ResponseEntity.status(HttpStatus.NOT_FOUND)
                    .body(new ApiResponse<>(false, e.getMessage(), null));
